...
诗人、生平、地图、关系、更新、音频及收藏备份已集成至 `81f1b84`。Mac `swift test` 共 26 项全部通过；未签名 iOS `build-for-testing` 通过。合并代理执行的服务 HTTP 测试 5 项通过，覆盖更新、关系目录校验、受保护媒体及备份修订协议。音频真实播放/循环试听、UI 操作和手机私网链路仍未验收。

服务拟用 Mac localhost 8787 与 Tailscale HTTPS 8443，检查时两端口空闲；现有 Immich 443 映射保留。独立凭据已经生成于忽略目录及 Mac 私有配置，权限限制为本人访问，未写入源码、内容包或日志。服务尚未部署。
